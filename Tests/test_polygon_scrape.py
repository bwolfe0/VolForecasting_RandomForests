...
def test_pull_prices_1():
    res = pull_prices(dt.date(2023,11,27),dt.date(2023,11,28),num_strikes=3)
    assert 454.48 == res['stock_close'] #Test known values
    assert .69 == res['call_close'][0]
    assert .64 == res['put_close'][0]

def test_pull_prices_2():
    res = pull_prices(dt.date(2024,2,5), dt.date(2024,2,6),num_strikes=3)
...
def test_calculate_IV():
    res = calculate_IV(dt.date(2020,9,1), trading_days,r,num_strikes=3)
    assert pytest.approx(0.1016, rel=1e-3) == res['avg_IV'] #Test known value

def test_calculate_IV_exceptions():
    with pytest.raises(ValueError):
        calculate_IV(dt.date(2023,12,3),trading_days,r)
        calculate_IV(5)
