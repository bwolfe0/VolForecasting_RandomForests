...
class  Test_BS_func(unittest.TestCase):
    ...
    def test_bs_price(self):
        self.assertAlmostEqual(3.97009, BS_functions.GetCall(S=30,K=28,T=.2,r=.025,sigma=.53816), places=4)
        with self.assertRaises(ValueError):
            BS_functions.GetCall(S=-5,K=28,T=.2,r=.025,sigma=.53816)
    ...
    def test_find_vol(self):
        self.assertAlmostEqual(.53816,BS_functions.GetIV(target_value=3.97,S=30,K=28,T=.2,r=.025,sigma_guess=.5)['Sigma'],places=4)
        with self.assertRaises(ValueError):
